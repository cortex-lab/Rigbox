classdef DummySignalsExp < handle
  ...
  methods
    function obj = DummySignalsExp(paramStruct, rig)
      clock = rig.clock;
      clockFun = @clock.now;
      ...
      obj.Audio.Devices = @aud.dummyDev;
      obj.Events = sig.Registry(clockFun);
      %% configure signals
      net = sig.Net;
      obj.Net = net;
      obj.Visual = StructRef; %net.subscriptableOrigin('visual'); 
...
%       obj.Occ = vis.init(obj.StimWindowPtr);
      ...
      obj.Wheel = obj.Data.inputs.wheelValues;
      if isfield(obj.Data.inputs, 'lickValues')
        obj.LickDetector = obj.Data.inputs.lickValues;
      end
    end
