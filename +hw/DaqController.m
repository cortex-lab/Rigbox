classdef DaqController < handle
    ...
    % 2018-07 KJM added support for parallel digital output
    
    properties
        AnalogChannelNames = {} % name to refer to each channel
        ...
        AnalogSignalGenerators = hw.PulseSwitcher.empty
        AnalogDaqIds = 'Dev1' % device ID's for each channel, e.g. 'Dev1'
        DaqAnalogChannelIds = {} % DAQ's ID for each channel, e.g. 'ao0'
        
        DigitalDaqId = 'Dev1';
        DigitalChannelName = '';
        DigitalSignalGenerator = hw.PulseSwitcher.empty
        DaqDigitalChannelIds = {}; % DAQ's ID for each digital channe e.g. 'port1/line1'
    ...
    properties (Dependent)
        ...
        NumAnalogChannels %Number of channels controlled
        NumDigitalChannels %Number of channels controlled
    ...
    methods
        function createDaqChannels(obj)
            % Create the analog session
            if isempty(obj.DaqSession)
                obj.DaqSession = daq.createSession('ni');
                obj.DaqSession.Rate = obj.SampleRate;
            end
            % Create the digital session
            if isempty(obj.DigitalDaqSession)
                obj.DigitalDaqSession = daq.createSession('ni');
            end
            
            % Set up analog channels
            n = obj.NumAnalogChannels;
            for channel_i = 1:n
                % Pick the appropriate daq for this channel
                if iscell(obj.AnalogDaqIds)
                    daqid = obj.AnalogDaqIds{channel_i};
                else
                    daqid = obj.AnalogDaqIds;
                end
                % Create the channel
            ...
            obj.CurrAnalogValue = v;
            
            % Set up digital channels
            n = obj.NumDigitalChannels;
            ...
            obj.DigitalDaqSession.outputSingleScan(v);
            obj.CurrDigitalValue = v;
        end
        
        function commandAnalog(obj, varargin)
            % Sends command signals to each channel
            %
            % command(channels, values)                                      % I think this first usage is not supported ~KJM 7-29-2019
            ...
            n = size(values, 2);
            if n > 0
                gen = obj.AnalogSignalGenerators(1:n);
                ...
                if any(any(values(:)~=0))
                    queue(obj, channelNames(1:n), waveforms(1:n));
                    if foreground
        ...
        function command_digital(obj, values)
            
            waveforms = obj.DigitalSignalGenerator.waveform(obj.SampleRate, values);
            obj.DigitalDaqSession.outputSingleScan(waveforms);
            
        end
        
        function v = get.NumAnalogChannels(obj)
            v = numel(obj.DaqAnalogChannelIds);
        end
        
        function v = get.NumDigitalChannels(obj)
            v = numel(obj.DaqDigitalChannelIds);
        end
        ...
        function v = get.Value(obj)
            v = [obj.CurrAnalogValue, obj.CurrDigitalValue];
        end
        
        function set.Value(obj, v)
            assert(numel(v) == obj.NumAnalogChannels + obj.NumDigitalChannels, ...
                'Must supply values for all channels');
            readyWait(obj);
            
            if obj.NumAnalogChannels > 0
                v_analog = v(1:obj.NumAnalogChannels);
                obj.DaqSession.outputSingleScan(v_analog);
            else
                v_analog = [];
            end
            
            if obj.NumDigitalChannels > 0
                v_digital = v((obj.NumAnalogChannels + 1) : end);
                obj.DigitalDaqSession.outputSingleScan(v_digital);
            else
                v_digital = [];
            end
            
            obj.CurrAnalogValue = v_analog;
            obj.CurrDigitalValue = v_digital;
        end
