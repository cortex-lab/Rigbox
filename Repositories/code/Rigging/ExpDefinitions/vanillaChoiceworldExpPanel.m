...
%             time = datenum(updates(ui).timestamp);
    ...
    function expUpdate(obj, rig, evt)
      ...
      switch type
        case 'signals' %queue signal updates
          ...
          %%%% AP: UPDATE PLOTS %%%%
          stimAzimithIdx = strmatch('events.stimAzimuth',{evt.Data.name});
          if ~isempty(stimAzimithIdx)
              curr_stim_position = evt.Data(stimAzimithIdx).value;
              set(obj.visStimPlot.stim,'XData',curr_stim_position);
              obj.visStimPlot.axes.XLim = [-180 180];
              obj.visStimPlot.axes.YLim = [-90,90];
          end
          ...
          if ~isempty(sessionPerformanceIdx)
              ...
              set(obj.performancePlot.dots,'XData',conditions(~isnan(leftward)), ...
                  'YData',leftward(~isnan(leftward)));
              set(obj.performancePlot.line,'XData',conditions(~isnan(leftward)), ...
                  'YData',leftward(~isnan(leftward)));
    ...
    obj.performancePlot.axes = bui.Axes(obj.CustomPanel);
    ...
    % Plot the visual stim as a moving dot
    obj.visStimPlot.axes = bui.Axes(obj.CustomPanel);
    obj.visStimPlot.axes.XLim = [-180 180];
    obj.visStimPlot.axes.YLim = [-20,20];    
    obj.visStimPlot.stim = plot(0,0,'ok','MarkerSize',20);   
    obj.visStimPlot.NextPlot = 'add';
